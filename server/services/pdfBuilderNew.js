...
module.exports = async function pdfBuilder(req) {
    ...
    const renderLabel = (doc, x, y, barcodeSvg) => {
        ...
        // рамка
       if(paper==='a4')  doc.rect(0, 0, labelWidthPt, labelHeightPt).lineWidth(1).stroke();
        ...
        if (showNoReturn === 'true') {
            if (heightMMNum<28) {
                // lines.push('Товар не подлежит ');
                // lines.push('обязательной ');
                // lines.push('сертификации');
                // lines.push('Товар не подлежит \n обязательной \n сертификации');
                linesLenght+=3
            }else{
                // lines.push('Товар не подлежит');
                // lines.push('обязательной сертификации');
                linesLenght+=2
            }
        }
        ...
        SVGtoPDF(doc, barcodeSvg, labelWidthPt / 2 - barcodeDims.width / 2, barcodeY, { width: barcodeDims.width, height: barcodeDims.height });

        let fontSize = heightMMNum < 30 ? Math.max(5, Math.min(10, (labelHeightPt - barcodeY) / (linesLenght + 2))) : Math.max(5, Math.min(10, 0.7*(labelHeightPt - barcodeY) / (linesLenght )));
        ...
        console.log("labelHeightPt ",labelHeightPt);
        console.log("calcs", (labelHeightPt - barcodeY) / (lines.length + 3));
        ...
        lines.forEach(line => {
            if (textY+fontSize<labelHeightPt) {
                doc.text(line, labelWidthPt / 100*15, textY, { width: labelWidthPt * 0.5, lineBreak: false,lineGap:0 });
            textY +=heightMMNum <= 30? fontSize :fontSize *1.1
            }
        });
        ...
        if (showNoReturn === 'true') {
            ...
            if (textY+noRet.length*fontSize<labelHeightPt) {
                    noRet.forEach(noRetElem=>{
                        doc.text(noRetElem, labelWidthPt / 100*15, textY, { width: labelWidthPt * 0.5, lineBreak: false,lineGap:0 })
                        textY += fontSize * 1;
                    })
                }
        }
        ...
        if (frame && frame !== 'None') {
            const framePath = path.join(__dirname, '../public/frames', frame);
                const frameSVG = fs.readFileSync(framePath).toString()
                .replace(/width="[^"]*"/i, '')
        .replace(/height="[^"]*"/i, '')
        .replace(/<svg[^>]+/, match => {
            return match.includes('preserveAspectRatio')
                ? match.replace(/preserveAspectRatio="[^"]*"/i, 'preserveAspectRatio="none"')
                : match.replace(/<svg/i, '<svg preserveAspectRatio="none"');
        });
        console.log("Width",widthMMNum);
        
                SVGtoPDF(doc, frameSVG, 0, 0, { width: mmToPt(widthMMNum) , height: mmToPt(heightMMNum) })
                //  fsPromises.unlink(frameSVG);
            }
        if (showEac === 'true') {
        ...
        if (req.file)  addLogo(doc, req.file.path, logoWidth, widthMMNum, heightMMNum, logoXPercent, logoYPercent)
        doc.restore();
    };

    const docOptions = paper === 'a4'
        ? { size: [a4WidthPt, a4HeightPt], margin: 1 }
        : { size: [labelWidthPt, labelHeightPt], margin: 0 };
    ...
    if (paper === 'thermal' ) {
        for (let i = 0; i < amount; i++) {
            ...
            if (i < amount - 1) doc.addPage({ size: [labelWidthPt, labelHeightPt], margin: 0 });
        }
    } else {
        ...
        for (let i = 0; i < amount; i++) {
            ...
            if (i > 0 && indexInPage === 0) doc.addPage({ size: [a4WidthPt, a4HeightPt], margin: 0 });
