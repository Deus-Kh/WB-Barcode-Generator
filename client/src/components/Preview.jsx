...
const Preview = ({ formData, logoPreview, previewUrl, setFormData }) => {
    ...
    useEffect(() => {
        ...
        const renderBarcode = async () => {
            ...
            if ((formData.showColor && formData.color) || (formData.showSize === "true" && formData.size)) {
                lines.push(
            ...
            if (formData.showCustomText && formData.customText) lines.push(formData.customText);
            ...
            const marginTop = Math.min(8, Math.max(2, Math.floor(formData.heightMm) / (linesLenght + 2)));
        ...
        const renderText = (barcodeBottomY) => {
            ...
            const fontSize = formData.heightMm < 30 ? 5 : Math.max(5, Math.min(8, (mmToPt(formData.heightMm) - barcodeBottomY) / ((linesLenght + 2)/0.75)));
            console.log("font Size", fontSize);
            console.log("aviable",mmToPt(formData.heightMm) - barcodeBottomY);
            

            ctx.font = `normal ${fontSize}pt  ${formData.font.replace('.ttf', '').replace(/\b\w/g, c => c.toUpperCase())}`;
            ...
            lines.forEach(line => {
                if (textY + fontSize < mmToPt(formData.heightMm)) {
                    wrapText(line, widthPx / 100 * 12, textY, mmToPt(formData.widthMm) * 0.55)
                }
                textY += fontSize * 1.2;
            });
            ...
            if (formData.showNoReturn) {
                ...
                if (textY + noRet.length * fontSize  < mmToPt(formData.heightMm)) {
                    noRet.forEach((noRetElem) => {
                        wrapText(noRetElem, widthPx / 100 * 12, textY, mmToPt(formData.widthMm) * 0.55)
                        textY += fontSize * 1.2;
                    });
                }
            }
