...
function App() {
  ...
  useEffect(() => {
    ...
    const renderBarcode = async () => {
      const barcodeHeight = formData.heightMm < 30 ? 0.25 : 0.4; // Увеличено
      ...
      try {
        await bwipjs.toCanvas(tempCanvas, {
          ...
          includetext: true,
          textxalign: 'center',
          textsize: 8, // Увеличено
        });
        const barcodeDims = { width: tempCanvas.width * barcodeHeight, height: tempCanvas.height * barcodeHeight };
        ...
        if (barcodeYPx >= 0 && barcodeYPx + barcodeDims.height <= heightPx) {
          ctx.drawImage(tempCanvas, widthPx / 2 - barcodeDims.width / 2, barcodeYPx, barcodeDims.width, barcodeDims.height);
          return (barcodeYPx + barcodeDims.height) / scaleFactor; // Нижняя граница штрихкода в мм
        } else {
    ...
    const renderText = (barcodeBottomY) => {
      ...
      if (formData.showArticle && formData.article) lines.push(`Article: ${formData.article}`);
      if ((formData.showColor && formData.color) || (formData.showSize && formData.size)) {
      ...
      if (formData.showExpirationDate && formData.expirationDate) lines.push(`Expiration date: ${formData.expirationDate}`);
      if (formData.showCountry && formData.country) lines.push(`Country: ${formData.country}`);
      if (formData.showBrand && formData.brand) lines.push(`Brand: ${formData.brand}`);
      if (formData.showNoReturn) lines.push('Товар не подлежит возврату');
      ...
      const availableHeight = formData.heightMm - barcodeBottomY - marginTop - marginBottom;
      const fontSize = Math.max(4, Math.min(8, availableHeight / (lineCount * 1)));

      ctx.font = `${fontSize }px Helvetica`;
      ctx.fillStyle = 'black';
      let textY = barcodeBottomY * scaleFactor + marginTop * scaleFactor;
      console.log('Text: y=', textY, 'fontSize=', fontSize, 'lines=', lines, 'availableHeight=', availableHeight, 'marginTop=', marginTop, 'marginBottom=', marginBottom);
      if (availableHeight > 0 && textY + fontSize * scaleFactor * 1.2 * lineCount <= heightPx) {
        lines.forEach(line => {
          ctx.fillText(line, 5 * scaleFactor, textY);
          textY += fontSize *  1;
        });
      } else {
    ...
    const renderEacAndLogo = () => new Promise(resolve => {
      ...
      if (formData.showEac) {
        ...
        eacImg.src = REACT_APP_SERVER_URL+'/frames/eac.svg';
        eacImg.onload = () => {
          const eacX = 5 * scaleFactor;
          const eacY = (formData.heightMm - 10) * scaleFactor;
          ctx.drawImage(eacImg, eacX, eacY, 8 * scaleFactor, 8 * scaleFactor);
      ...
      if (formData.logo) {
        ...
        logoImg.src = URL.createObjectURL(formData.logo);
        logoImg.onload = () => {
          const logoXPx = (formData.logoXPercent * widthPx) / 100;
          const logoYPx = (formData.logoYPercent * heightPx) / 100;
          ctx.drawImage(logoImg, logoXPx, heightPx - logoYPx - formData.logoWidth * scaleFactor, formData.logoWidth * scaleFactor, formData.logoWidth * scaleFactor);
          console.log('Logo rendered at:', { x: logoXPx, y: heightPx - logoYPx - formData.logoWidth * scaleFactor });
          checkDone();
        };
        logoImg.onerror = () => {
